...
public class Bike implements Vehicle {
    ...
    public void speedUp(int speedIncrement) {
        if (this.started) {
            if ((currentSpeed + Math.abs(speedIncrement)) < MAX_BIKE_SPEED) {
                this.currentSpeed += Math.abs(speedIncrement);
                System.out.println(SPEED_UP);
            } else {
                this.currentSpeed = MAX_BIKE_SPEED;
    ...
    public String toString() {
        return this.started ? String.format("Current speed of the bike: %d km/h", this.currentSpeed) : "The bike is Off.";
    }
}
